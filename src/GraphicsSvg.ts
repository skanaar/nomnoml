...
export function GraphicsSvg(document?: HTMLDocument): ISvgGraphics {
  ...
  return {
    ...
    arcTo: function (x1, y1, x2, y2, radius) {
      if (!inPathBuilderMode) throw new Error('can only be called after .beginPath()')
    ...
    fillText: function (text, x, y) {
      return el(
        'text',
        {
          ...
          'text-anchor':
            getDefined(current, (e) => e.attr['text-align']) === 'center' ? 'middle' : undefined,
        },
        text
      )
    },
      ...
      if (ctx) {
        ...
        if (current)
          ctx.font = `${getDefined(current, (e) => e.attr['font-weight'])} ${getDefined(
            current,
            (e) => e.attr['font-style']
          )} ${getDefined(current, (e) => e.attr['font-size'])} ${getDefined(
            current,
            (e) => e.attr['font-family']
          )}`
        else
